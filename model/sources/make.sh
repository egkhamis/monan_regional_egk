...
export DIRhome=`cd ../../;pwd`
export DIRroot=`cd ../../;pwd`

export MONAN_SRC_DIR=${DIRhome}/sources/${version_model}
...
cp -f ${MONAN_SRC_DIR}/bin/init_atmosphere_model ${MONAN_EXE_DIR}/exec/
cp -f ${MONAN_SRC_DIR}/bin/atmosphere_model      ${MONAN_EXE_DIR}/exec/
cp -f ${MONAN_SRC_DIR}/bin/build_tables          ${MONAN_EXE_DIR}/exec/
cp -f ${MONAN_SRC_DIR}/bin/init_atmosphere_model ${MONAN_EXE_PRE}/exec/

if [ -s "${MONAN_EXE_DIR}/exec/init_atmosphere_model" ] && [ -e "${MONAN_EXE_DIR}/exec/atmosphere_model" ]; then
    echo ""
    echo -e "\033[1;32m==>\033[0m Files init_atmosphere_model and atmosphere_model generated Successfully in ${MONAN_SRC_DIR}/bin and copied to ${MONAN_EXE_DIR}/exec !"
    echo
else
